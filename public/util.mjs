...
export const tailwinds = {
    ...
    itemDeleteButton: "h-full w-1/4 text-center text-transparent select-none hover:text-white",
...
export const functions = {
    ...
    let nums = [""];
    let ptr = 0;
    for (let chr = 0; chr < rgb.length; chr++){
        if (!isNaN(rgb[chr])) nums[ptr] += rgb[chr];

        else if (rgb[chr] == ','){
            nums[ptr] = parseInt(nums[ptr]);
            nums.push('');
            ptr++;
        }
     ...
     * @param {Array} nums 
     */
    RGBtoHex : function (nums){
        nums.forEach((el, i) => {
            el = el.toString(16).toUpperCase();
            nums[i] = el.length < 2 ? `0${el}` : el;
        });
        return `#${nums[0]}${nums[1]}${nums[2]}`;
    },

    findColor : function(value){
        const {RGBtoHex, parseRGB} = this;
        if (value.slice(0, 3) == "rgb") value = RGBtoHex(parseRGB(value));

        for (let color in tailwindColors){
            for (let hexValue in tailwindColors[color]){
                if (tailwindColors[color][hexValue] == value){
                    return color;
                }
            }
        }
